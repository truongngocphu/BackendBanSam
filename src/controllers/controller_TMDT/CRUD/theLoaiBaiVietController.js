const TheLoaiBaiViet = require("../../../model/ModelBanSam/TheLoaiBaiViet");
// CREATE
exports.createTheLoaiBaiViet = async (req, res) => {
  try {
    const doc = new TheLoaiBaiViet(req.body);
    await doc.save();
...
exports.getAllTheLoaiBaiViet = async (req, res) => {
  try {
    const { 
      page = 1, 
      limit = 1000, 
    ...
    const data = await TheLoaiBaiViet.find(query)
      .sort(sortOptions)
      .limit(limit * 1)
      .skip((page - 1) * limit);

    const count = await TheLoaiBaiViet.countDocuments(query);
...
// READ (One by ID)
exports.getTheLoaiBaiVietById = async (req, res) => {
  try {
    const doc = await TheLoaiBaiViet.findById(req.params.id);
    if (!doc) {
...
// UPDATE by ID
exports.updateTheLoaiBaiVietById = async (req, res) => {
  try {
    const doc = await TheLoaiBaiViet.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
      runValidators: true,
    });
    if (!doc) {
...
// DELETE by ID
exports.deleteTheLoaiBaiVietById = async (req, res) => {
  try {
    const doc = await TheLoaiBaiViet.findByIdAndDelete(req.params.id);
    if (!doc) {
      return res.status(404).json({ success: false, message: "Không tìm thấy" });
    }
